...
Policy and assignment inventory tools support pagination with `pageSize` and `pageToken`: `list_device_groups`, `list_policies`, `list_policy_assignments`, and `list_ip_sets`. Use `get_policy` with `decodePolicyData=true` to recursively decode base64 `PolicyData` blobs into `_mcpDecodedPolicyData` for troubleshooting product settings such as firewall/network protection. If decoded policy data contains an extra payload after the first JSON object, the first JSON object is returned and the extra data is reported with `trailingBytes` and `trailingPreview`. `list_ip_sets` is limited by ESET to Common features policies; unsupported policies may return HTTP 400, while HTTP 500 with an empty body should be treated as an upstream ESET failure for the policy or tenant.
